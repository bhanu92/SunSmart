...
router.get("/getDeviceData", function(req, res, next) {
  ...
  DeviceModel.find(deviceModelQuery, function(err, deviceModelData) {
    if (err) {
      console.log(err.message);
      return res.status(201).send(err.message);
    }
    else {
      var _deviceID = deviceModelData.deviceId;
      var start = new Date();
      ...
      DataModel.find(dataModelQuery, function(err, dataModelData) {
        if (err) {
          console.log(err.message);
          return res.status(201).send(err.message);
        }
